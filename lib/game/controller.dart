...
class Game {
  ...
  Game({bool skipBestTurnStage = false})
      : this.withPlayers(
          generatePlayers(),
  ...
  BaseGameState? get nextStateAssumption {
    switch (_state.stage) {
      ...
      case GameStage.bestTurn:
        final state = _state as GameStateBestTurn;
        return GameStateWithPlayer(
          stage: GameStage.nightLastWords,
          day: state.day,
  ...
  void togglePlayerSelected(int playerNumber) {
    ...
    if (state is GameStateBestTurn) {
      if (state.playerNumbers.contains(playerNumber)) {
        state.playerNumbers.remove(playerNumber);
      } else if (state.playerNumbers.isEmpty) {
        // workaround for adding a player to immutable `const []`
        _state = GameStateBestTurn(
          day: state.day,
          currentPlayerNumber: state.currentPlayerNumber,
          playerNumbers: [playerNumber],
        );
      } else if (state.playerNumbers.length < 3) {
        state.playerNumbers.add(playerNumber);
      }
      return;
    }
  }
  ...
  BaseGameState _handleEndOfNight() {
    final state = _state as GameStateNightCheck;
    final killedPlayerNumber = state.thisNightKilledPlayerNumber;
